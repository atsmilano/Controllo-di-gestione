...
}, "a65c581e-62d9-473d-a156-c1128dev04cb");
...
"refresh" : function (id, show) {
	...
	if(dialogs.get(id).params.framework) {
		return;
	}
...
"makeInstance" : function (id) {
	...
	if(tmp_params.framework) {
		var tmp = jQuery('<div class="modal fade" role="dialog">'
					 ...
					 +      '</div>'
					 +      '<div "ffWidget_dialog_container_' + id + '" class="modal-body">'
					 +      '</div>'
...
"removeDlgBt" : function(id, data) {
	var instance = dialogs.get(id).instance;
	var widget = (dialogs.get(id).params.framework
		? instance
		: instance.dialog("widget")
	);
