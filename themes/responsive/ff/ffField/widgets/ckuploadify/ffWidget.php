...
class ffWidget_ckuploadify extends ffCommon
{
	...
	{
		global $plgCfg_ckuploadify_UseOwnSession;
...
//		$this->tpl[$tpl_id]->set_var("properties", $Field->getProperties());
        ...
        if(strlen($Field->widget_path))
            ...
            $this->tpl[$tpl_id]->set_var("widget_path", "/themes/responsive/ff/ffField/widgets/ckuploadify"); 
        ...
        if($Field->extended_type == "File") {
			...
			if($temp_path == "/") {
				$this->tpl[$tpl_id]->set_var("relative_path", "");
			} else {
				$this->tpl[$tpl_id]->set_var("relative_path", $folder);
			}
			
			$tmp = md5($Field->source_SQL);
			if ($plgCfg_ckuploadify_UseOwnSession || $Field->actex_use_own_session)
				session_start();
