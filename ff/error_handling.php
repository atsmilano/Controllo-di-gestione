...
if (!defined("FF_ERRORS_HANDLED"))
    define("FF_ERRORS_HANDLED", E_USER_ERROR | E_USER_WARNING | E_USER_NOTICE /* | E_ERROR | E_WARNING */);
if (!defined("FF_ERROR_HANDLER_SUPPRESS_MASK"))
    define("FF_ERROR_HANDLER_SUPPRESS_MASK", 0); // wich errors must be ignored. 0 to disable
...
if (!defined("FF_ERROR_HANDLER_CUSTOM_TPL"))
    define("FF_ERROR_HANDLER_CUSTOM_TPL", ""); // path relativo a FF_DISK_PATH
...
if (!defined("FF_ERROR_HANDLER_LOG_PATH"))
    define("FF_ERROR_HANDLER_LOG_PATH", FF_DISK_PATH . "/ff_errors");

set_error_handler("ffErrorHandler::errorHandler", FF_ERRORS_HANDLED);
error_reporting(error_reporting() ^ FF_ERRORS_HANDLED);
...
abstract class ffErrorHandler
{
    ...
    {
        $id = uniqid(rand(), true);
        ffErrorHandler::$errors_objects = array();
        ffErrorHandler::$errors_arrays = array();

        ffErrorHandler::$errors_handled[$id]["description"] = $errdes;
        ffErrorHandler::$errors_handled[$id]["context"] = NULL;
        ffErrorHandler::$errors_handled[$id]["variables"] = $variables;
        $tmp = get_defined_constants();
        ffErrorHandler::compactConstants($tmp);
        ffErrorHandler::$errors_handled[$id]["constants"] = $tmp;
    ...
    static function errorHandler($errno, $errstr, $errfile, $errline, $errcontext = null)
    {
        ...
        switch ($errno & (E_ALL ^ FF_ERROR_HANDLER_SUPPRESS_MASK)) {
            case E_ERROR:
                //$error_description = "<b>E_ERROR - Fatal run-time error</b><br />These indicate error that can not be recovered from, such as a memory allocation problem. Execution of the script is halted<br /><br />";
                $error_description = "<b>E_ERROR - Fatal run-time error</b><br />";
                $halt_script = true;
                break;

            case E_WARNING:
                //$error_description = "<b>E_WARNING - Run-time warning (non-fatal errors)</b><br />Execution of the script is not halted<br /><br />";
                $error_description = "<b>E_WARNING - Run-time warning (non-fatal errors)</b><br />";
                break;

            case E_PARSE:
                //$error_description = "<b>E_PARSE - Compile-time parse error</b><br />Parse errors should only be generated by the parser<br /><br />";
                $error_description = "<b>E_PARSE - Compile-time parse error</b><br />";
                $errstr = uniqid(rand(), true);
                $halt_script = true;
                break;

            case E_NOTICE:
                //$error_description = "<b>E_NOTICE - Run-time notice</b><br />Indicate that the script encountered something that could indicate an error, but could also happen in the normal course of running a script<br /><br />";
                $error_description = "<b>E_NOTICE - Run-time notice</b><br />";
                $errstr = uniqid(rand(), true);
                break;

            case E_CORE_ERROR:
                //$error_description = "<b>E_CORE_ERROR - Fatal error that occur during PHP's initial startup</b><br />This is like an E_ERROR, except it is generated by the core of PHP.  PHP >= 4 only<br /><br />";
                $error_description = "<b>E_CORE_ERROR - Fatal error that occur during PHP's initial startup</b><br />";
                $errstr = uniqid(rand(), true);
                $halt_script = true;
                break;

            case E_CORE_WARNING:
                //$error_description = "<b>E_CORE_WARNING - Warning (non-fatal error) that occur during PHP's initial startup</b><br />This is like an E_WARNING, except it is generated by the core of PHP.  PHP >= 4 only<br /><br />";
                $error_description = "<b>E_CORE_WARNING - Warning (non-fatal error) that occur during PHP's initial startup</b><br />";
                $errstr = uniqid(rand(), true);
                break;

            case E_COMPILE_ERROR:
                //$error_description = "<b>E_COMPILE_ERROR - Fatal compile-time error</b><br />This is like an E_ERROR, except it is generated by the Zend Scripting Engine.  PHP >= 4 only<br /><br />";
                $error_description = "<b>E_COMPILE_ERROR - Fatal compile-time error</b><br />";
                $errstr = uniqid(rand(), true);
                $halt_script = true;
                break;

            case E_COMPILE_WARNING:
                //$error_description = "<b>E_COMPILE_WARNING - Compile-time warning (non-fatal error)</b><br /> This is like an E_WARNING, except it is generated by the Zend Scripting Engine.  PHP >= 4 only<br /><br />";
                $error_description = "<b>E_COMPILE_WARNING - Compile-time warning (non-fatal error)</b><br />";
                $errstr = uniqid(rand(), true);
                break;

            case E_USER_ERROR:
                //$error_description = "<b>E_USER_ERROR - User-generated error message</b><br />This is like an E_ERROR, except it is generated in PHP code by using the PHP function trigger_error()<br /><br />";
                $error_description = "<b>E_USER_ERROR - User-generated error message</b><br />";
                //$error_description = "<b>CRITICAL ERROR</b><br />";
                $halt_script = true;
                break;

            case E_USER_WARNING:
                //$error_description = "<b>E_USER_WARNING - User-generated warning message</b><br />This is like an E_WARNING, except it is generated in PHP code by using the PHP function trigger_error()<br /><br />";
                $error_description = "<b>E_USER_WARNING - User-generated warning message</b><br />";
                //$error_description = "<b>WARNING</b><br />";
                break;

            case E_USER_NOTICE:
                //$error_description = "<b>E_USER_NOTICE - User-generated notice message</b><br />This is like an E_NOTICE, except it is generated in PHP code by using the PHP function trigger_error()<br /><br />";
                $error_description = "<b>E_USER_NOTICE - User-generated notice message</b><br />";
                //$error_description = "<b>NOTICE</b><br />";
                break;

            default:
                return;
            //$error_description = "<b>Unknown Error ($errno)</b><br />This error is unknown (unkown errors happen in PHP).";
        }
        ...
        switch ($errno) {
            ...
            default:
                $id = uniqid(rand(), true);
                ffErrorHandler::$errors_handled[$id]["description"] = $errstr;
                ffErrorHandler::$errors_handled[$id]["context"] = NULL;
                ffErrorHandler::$errors_handled[$id]["variables"] = $errcontext;
                $tmp = get_defined_constants();
                ffErrorHandler::compactConstants($tmp);
                ffErrorHandler::$errors_handled[$id]["constants"] = $tmp;
                $errstr = $id;
        ...
        if (/* $halt_script && */self::$whenErrorDo500)
        ...
        if (self::logEnabled()) {
            ...
            self::$log_fp = @fopen(self::$log_path . "/" . $errstr . ".log.html", "a");
        }
...
EOD
        ...
        self::out('<div style="display: block; background-color: #AAAAAA; border: 1px solid #FF0000; padding: 10px;font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 12px;">
			<div style="display: block; background-color: #FF0000; border: 0; padding: 5px; color: #FFFFFF;">
				<b>-=| FORMS FRAMEWORK |=-</b> ERROR HANDLED #' . (self::logEnabled() ? $errstr : ($error_id + 1)) . '
			</div>');

        // DISPLAY HEADER INFORMATION
        self::out('<p>' . $error_description . self::$errors_handled[$errstr]["description"] . '</p>');
    ...
    static function removeGlobals($params)
    {
        $res = null;
        if (is_array($params) && count($params)) {
            ...
            foreach ($params as $key => $value) {
                if (!(
                        (isset($params["_ENV"][$key]) && $params["_ENV"][$key] == $value) || (isset($params["_SERVER"][$key]) && $params["_SERVER"][$key] == $value) || (isset($params["_COOKIE"][$key]) && $params["_COOKIE"][$key] == $value) || (isset($params["_POST"][$key]) && $params["_POST"][$key] == $value) || (isset($params["_GET"][$key]) && $params["_GET"][$key] == $value) || (isset($params["_FILES"][$key]) && $params["_FILES"][$key] == $value) || (isset($params["_SESSION"][$key]) && $params["_SESSION"][$key] == $value) || (substr($key, 0, strlen(APPID)) == APPID) || $key == "HTTP_ENV_VARS" || $key == "HTTP_SERVER_VARS" || $key == "HTTP_COOKIE_VARS" || $key == "HTTP_POST_VARS" || $key == "HTTP_GET_VARS" || $key == "HTTP_FILES_VARS" || $key == "HTTP_SESSION_VARS" || $key == "GLOBALS"
                        ))
                    $res[$key] = $value;
                //unset($params[$key]);
            }
    ...
    static function structPrint(&$arg, $recursion = 0, $display_lines = true)
    {
        if (!$recursion)
            self::out("<code>");
        ...
        foreach ($vars as $key => $value) {
            ...
            if (is_object($value)) {
                ...
                if (
                        (string) $key != "FormsErrorsHandled" && (string) $key != "FormsErrorsObjects" && (string) $key != "FormsErrorsArrays"
                ) {
                    $bFind = FALSE;
                ...
                if (
                        strpos(strtolower($key), "password") !== FALSE || $key === "_crypt_Ku_" || $key === "_crypt_KSu_"
                )
