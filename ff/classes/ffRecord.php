 ...
 * @link http://www.formsphpframework.com
 */
if (!defined("FF_ENABLE_MEM_PAGE_CACHING"))
...
abstract class ffRecord_base extends ffCommon
{
    ...
    function retrieve_fields()
    {
        ...
        if ($this->record_exist && ($this->first_access || $this->ever_reload_data) && $this->src_table) { // retrieve fields from DB
                                    ...
                                    if (is_array($this->form_fields[$key]->multi_fields) && count($this->form_fields[$key]->multi_fields)) {
                                        ...
                                        if ($this->db[0]->isSetField($this->form_fields[$key]->get_data_source())) {
                                            $tmp = ($this->form_fields[$key]->crypt ?
                                                    ff_getDecryptedField($this->db[0], $this->form_fields[$key]->get_data_source(), $this->form_fields[$key]->base_type) : $this->db[0]->getField($this->form_fields[$key]->get_data_source(), $this->form_fields[$key]->base_type));
        ...
        } else if ((!$this->record_exist || !$this->src_table) && $this->first_access) { // set proper default values
        ...
        } else { // retrieve fields from Form
            foreach ($this->form_fields as $key => $FormField) {
                if (is_array($this->form_fields[$key]->multi_fields) && count($this->form_fields[$key]->multi_fields)) {
                    ...
                    case "File":
                        ...
                        if ($this->parent[0]->retrieve_param($this->id, $this->form_fields[$key]->id . "_delete") == "delete") {
                            ...
                            $this->form_fields[$key]->file_tmpname = "";
                                ...
                                $this->parent[0]->isset_param($this->id, $this->form_fields[$key]->id . "_file") && is_array($file_array) && isset($file_array["tmp_name"]) && strlen($file_array["tmp_name"])
                        ) {
                            /* if (
                            ...
                            if (!$this->form_fields[$key]->file_max_size ||
                                ...
                                if ($this->form_fields[$key]->file_full_path) {
                                    if (
                                            substr(strtolower($this->form_fields[$key]->value->getValue()), 0, 7) != "http://" && substr(strtolower($this->form_fields[$key]->value->getValue()), 0, 8) != "https://" && substr($this->form_fields[$key]->value->getValue(), 0, 2) != "//"
                                    ) {
        ...
        if (strlen($this->frmAction) && $this->frmAction != "delete" && $this->frmAction != "confirmdelete" && strpos($this->frmAction, "detail_") !== 0) {
            ...
            foreach ($this->form_fields as $key => $FormField) {
                ...
                if ($rc === null) {
                    ...
                    // corrispondency
                    if (
                            strlen($this->form_fields[$key]->compare) && $this->form_fields[$this->form_fields[$key]->compare]->value->ori_value != $this->form_fields[$this->form_fields[$key]->compare]->value_ori->ori_value && $this->form_fields[$key]->getValue() !== $this->form_fields[$this->form_fields[$key]->compare]->getValue()
                    ) {
                        $this->form_fields[$key]->contain_error = true;
                        $this->contain_error = true;
        ...
        switch (isset($this->default_actions[$this->frmAction]) ? $this->default_actions[$this->frmAction] : $this->frmAction) {
                        ...
                        if ($this->form_fields[$key]->store_in_db == true && !strlen($this->form_fields[$key]->compare) &&
                            ...
                            if (is_array($this->form_fields[$key]->multi_fields) && count($this->form_fields[$key]->multi_fields)) {
                                ...
                                $fields .= "`" . $this->src_table . "`.`" . $this->form_fields[$key]->get_data_source(false) . "`"
                                        . " = "
                                        . ($processed_sql_value ?
                                        $tmpval : $this->db[0]->toSql($tmpval, $tmp_type)
                                        );
                            }
                        }
                    }
            ...
            case "delete":
                if ($this->url_delete !== null) {
                    $tmp_url = ffProcessTags($this->url_delete, $this->key_fields, $this->form_fields, "normal", $this->parent[0]->get_params(), rawurlencode($_SERVER['REQUEST_URI']), $this->parent[0]->get_globals(GET_GLOBALS_EXCLUDE_LIST), $this->db[0]);
