	...
	function connect($Database = null, $Host = null, $User = null, $Password = null, $force = false)
	{
		...
		if ($do_connect)
		{
			if (!$this->avoid_real_connect)
			{
                ...
                */
                if ($this->persistent)
                    $rc = @mysqli_real_connect($this->link_id, "p:" . $this->host, $this->user, $this->password, (FF_DB_MYSQLI_AVOID_SELECT_DB ? $this->database : null), null, null, (FF_DATABASE_USE_SSL === true ? MYSQLI_CLIENT_SSL : MYSQLI_CLIENT_FOUND_ROWS));
                else
                    $rc = @mysqli_real_connect($this->link_id, $this->host, $this->user, $this->password, (FF_DB_MYSQLI_AVOID_SELECT_DB ? $this->database : null), null, null, (FF_DATABASE_USE_SSL === true ? MYSQLI_CLIENT_SSL : MYSQLI_CLIENT_FOUND_ROWS));
                //**************************************************************              
			}
			else
			{                
