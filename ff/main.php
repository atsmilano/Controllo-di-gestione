...
if (!defined("__TOP_DIR__")) {
    define("__TOP_DIR__", (getenv("FF_TOP_DIR") && getenv("FF_TOP_DIR") != "1" ? getenv("FF_TOP_DIR") : __FF_DIR__
    ));
}
...
define("__PRJ_DIR__", (getenv("FF_PROJECT_DIR") && getenv("FF_PROJECT_DIR") != "1" ? getenv("FF_PROJECT_DIR")
        //ADDED-ATS sostituito $_SERVER["REDIRECT_FF_PROJECT_DIR"] con isset($_SERVER["REDIRECT_FF_PROJECT_DIR"] alla riga successiva
                : (isset($_SERVER["REDIRECT_FF_PROJECT_DIR"]) ? $_SERVER["REDIRECT_FF_PROJECT_DIR"] : __TOP_DIR__
                )
        ));
...
if (!defined("FF_UPDIR"))
    define("FF_UPDIR", "/uploads");
if (!defined("FF_SITE_UPDIR"))
    define("FF_SITE_UPDIR", FF_SITE_PATH . FF_UPDIR);
if (!defined("FF_DISK_UPDIR"))
    define("FF_DISK_UPDIR", FF_DISK_PATH . FF_UPDIR);

// Theme Management
// define base theme(s) location
if (!defined("FF_THEME_DIR"))
    define("FF_THEME_DIR", "/themes");
if (!defined("FF_THEME_DISK_PATH"))
    define("FF_THEME_DISK_PATH", __TOP_DIR__ . FF_THEME_DIR);
if (!defined("FF_THEME_SITE_PATH"))
    define("FF_THEME_SITE_PATH", FF_SITE_PATH . FF_THEME_DIR);

if (!defined("FF_MAIN_THEME"))
    define("FF_MAIN_THEME", "responsive");
...
if (!defined("FF_URLREWRITE_REMOVEHYPENS"))
    define("FF_URLREWRITE_REMOVEHYPENS", true);
if (!defined("FF_ORM_ENABLE"))
    define("FF_ORM_ENABLE", true);

if (!defined("FF_PREFIX"))
    define("FF_PREFIX", "ff_");
if (!defined("FF_SUPPORT_PREFIX"))
    define("FF_SUPPORT_PREFIX", "support_");
