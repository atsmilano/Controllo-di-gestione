...
try {
    $descrizione_introduttiva = StrategiaDescrizioneIntroduttiva::factoryFromAnnoBudget($anno);
...
if (count($prospettive_anno) > 0) {
    foreach ($prospettive_anno as $prospettiva) {
        $cm->oPage->AddContent("<p><strong>" . $prospettiva->nome . ":</strong> " . CoreHelper::stripTagsUTF8Encode($prospettiva->descrizione) . "</p>");
    }
} else {
