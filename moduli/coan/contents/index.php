...
$periodi_anno = CoanPeriodo::getAll();
if (count($periodi_anno) > 0){
    ...
    $cdc_standard_regionali = CoanCdcStandardRegionale::getAll();
    ...
    $distretti = CoanDistretto::getAll();
    ...
    $sql = "
		...
		FROM 
			...
			INNER JOIN (SELECT coan_cdc.* FROM coan_cdc WHERE " . $query_filter . ") AS coan_cdc ON coan_consuntivo_periodo.ID_cdc_coan = coan_cdc.ID
		WHERE
			coan_consuntivo_periodo.ID_periodo_coan = " . $db->toSql($periodo->id) . "			
		GROUP BY
			coan_conto.ID
		ORDER BY
			coan_fp_primo.ID,
