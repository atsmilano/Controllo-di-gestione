...
//in caso di inserimento viene creato il codice incrementale rispetto all'anno da attribuire all'obiettivo
else {
    $obiettivi_anno = array();
    $obiettivi_all = ObiettiviObiettivo::getAll(array("ID_anno_budget" => $anno->id), array(array("fieldname"=>"codice_incr_anno", "direction"=>"ASC")));
    foreach ($obiettivi_all as $obiettivo_anno){
        if ( $obiettivo_anno->data_eliminazione == null) {
            $obiettivi_anno[] = $obiettivo_anno;
        }        
    }
    ...
    if ($obiettivi_anno == null) {
        ...
        for ($i=1; $i<count($obiettivi_anno); $i++){            
            if((int)$ultimo_codice_incrementale + 1 !== (int)$obiettivi_anno[$i]->codice_incr_anno) {                  
                break;
            }
