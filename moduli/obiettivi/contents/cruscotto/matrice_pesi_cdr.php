		...
		foreach ($obiettivi_cdr_anno as $obiettivo_cdr) {
				...
				foreach($obiettivi_cdr_anno_figlio as $key => $obiettivo_cdr_anno_figlio){
                    if ($obiettivo_cdr->id_obiettivo == $obiettivo_cdr_anno_figlio->id_obiettivo){
                        ...
                        if ($obiettivo_cdr_padre == $obiettivo_cdr
                                && $cdr->codice !== $cdr_figlio->codice) {
				...
				if ($found == null){
                    if (($cdr_figlio->codice != $cdr->codice && !$obiettivo_cdr->isChiuso() && $user->hasPrivilege("resp_cdr_selezionato"))
                        || ($cdr_figlio->codice == $cdr->codice && $user->hasPrivilege("obiettivi_aziendali_edit") && ($obiettivo_cdr->isObiettivoCdrAziendale() || $obiettivo_cdr->isCoreferenza()))    
                        ){ 
