...
$grid_direzione_da_approvare_recordset = array();
$grid_direzione_competenza_recordset = array();
$tr_title_js = "";
foreach (RURichiesta::getAll(array("ID_anno_budget"=>$anno->id)) as $richiesta) {
    ...
    if ($id_stato_avanzamento >= 4 && $stato_avanzamento["esito"] !== "ko") {       
        ...
        if ($richiesta->isApprovazioneCompetenza($cdr, $anno, $data_riferimento, $piano_cdr)){
            $grid_direzione_da_approvare_recordset[] = $record;
        }
        else {
            $grid_direzione_competenza_recordset[] = $record;
        }            
    }
}

$oGrid = RURichiesta::getGridRichieste("richieste-approvazione", "Richieste da approvare", $grid_direzione_da_approvare_recordset);
...
$oGrid = RURichiesta::getGridRichieste("richieste-competenza", "Richieste approvate", $grid_direzione_competenza_recordset);
$oGrid->display_new = false;
$oGrid->display_delete_bt = false;
$oGrid->display_search = false;
$oGrid->use_search = false;
