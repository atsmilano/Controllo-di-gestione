...
foreach (RURichiesta::getAll(array("ID_anno_budget"=>$anno->id)) as $richiesta) {
    ...
    if ($id_stato_avanzamento >= 13) {       
        ...
        $grid_dg_rifiutate[] = $record;            
    }
}

$oGrid = RURichiesta::getGridRichieste("richieste-rifiutate", "Richieste di competenza rifiutate", $grid_dg_rifiutate, true);
