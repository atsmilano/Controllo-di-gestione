...
class cm extends ffCommon
{
    ...
    public function process()
    {
        ...
        if (
                CM_PAGECACHE && !$this->isXHR() && !defined("FF_URLPARAM_NOCACHE") && !defined("FF_URLPARAM_DEBUG") && !defined("CM_DONT_RUN") && strpos($this->path_info, "sitemap.xml") === false && !$cache_avoid_match && (!defined("ALLOW_PAGECACHE") || ALLOW_PAGECACHE)
        ) {
                        ...
                        if (CM_PAGECACHE_GROUPDIRS) {
                            ...
                            foreach ($itGroup as $fiGroup) {
                                ...
                                if (false !== ($fctime = @filectime($file))) {
                                    ...
                                    else if (!CM_PAGECACHE_LAST_VALID || $now < CM_PAGECACHE_LAST_VALID || $fctime >= CM_PAGECACHE_LAST_VALID
                                    ) {
        ...
        $this->oPage->jquery_ui_theme = ($this->layout_vars["framework_css"] ? false : "base"
                );
        ...
        if (CM_ENABLE_MEM_CACHING && CM_ENABLE_PATH_CACHE)
            $cm_router_matches = $this->cache->get("__cm_router_matches_" . $this->path_info . "__", $success);
        if ($success) {
            ...
            if (CM_ENABLE_MEM_CACHING && CM_ENABLE_PATH_CACHE)
                $this->cache->set("__cm_router_matches_" . $this->path_info . "__", null, serialize($this->router->matched_rules), "__cm_router__");
        }
        ...
        foreach ($this->router->matched_rules as $key => $match) {
            ...
            if (strlen($this->script_name)) {
                ...
                if (
                        end($path_parts) . ".html" == end($script_parts) || end($path_parts) . "." . FF_PHP_EXT == end($script_parts)
                ) {
        ...
        if (
                ALLOW_PAGECACHE && $this->cache_force_enable !== false && http_response_code() == 200 && !@file_exists($cache_dir . "/disk_fail") && (
                !CM_PAGECACHE_ASYNC || (
            ...
            if ($this->cache_force_max_age === null || $this->cache_force_expire === null) {
                ...
                if (CM_ENABLE_MEM_CACHING && CM_CACHE_ROUTER_MATCH)
                    $cm_cacherouter_matches = $this->cache->get("__cm_cacherouter_matches_" . $this->path_info . "__", $success);
                if ($success) {
                    ...
                    if (CM_ENABLE_MEM_CACHING && CM_CACHE_ROUTER_MATCH)
                        $this->cache->set("__cm_cacherouter_matches_" . $this->path_info . "__", null, serialize($this->cache_router->matched_rules), "__cm_router__");
                }
        ...
        if (is_array($this->loaded_applets) && count($this->loaded_applets)) {
            foreach ($this->loaded_applets as $key => $value) {
                ...
                if (isset($this->applets_components[$key]["_buffer_"])) {
                    ...
                    if (
                            is_array($this->applets_components) && count($this->applets_components) && is_array($this->applets_components[$key]) && count($this->applets_components[$key])
                    ) {
    ...
    function getLayoutByPath($layout_path)
    {
            ...
            do {
                ...
                if ($db2->nextRecord()) {
                    do {
                        ...
                        $layout_vars["meta"][$db2->getField("name", "Text", true)]["type"] = (strlen($db2->getField("type", "Text", true)) ? $db2->getField("type", "Text", true) : "name"
                                );
        ...
        if (CM_ENABLE_MEM_CACHING && CM_ENABLE_PATH_CACHE)
            $this->cache->set("__cm_getLayoutByPath_" . $layout_path . "__", null, $layout_vars, "__cm_layout__");
        return $layout_vars;
    }
