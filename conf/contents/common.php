...
function getUser($oPage)
{
    ...
    $menu_item = Modulo::getCurrentModuleMenuItem($request_uri);
   
    if (
        (
            $menu_item["hide"] == true
            || $menu_item == null
            || $menu_item["icon"] == MODULES_ICONHIDE
        ) && (
    ...
    if(!$cm->oPage->isXHR()) {
        ...
        $modulo = Modulo::getCurrentModuleMenuItem($url, false);
